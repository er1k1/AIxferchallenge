...
# initialize the label names for the STL-10 dataset
...
def read_all_images(path_to_data, max):
    """
    ...
    with open(path_to_data, 'rb') as f:
        ...
        everything = np.fromfile(f, dtype=np.uint8, count= max * SIZE)
...
def decode_predictions(preds, labelNames):
    results = []
    for pred in preds:
        top_indices = pred.argsort()[-3:][::-1]
        result = [{labelNames[i]: str(pred[i])} for i in top_indices]
